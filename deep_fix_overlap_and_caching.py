...
# 1. Add Cache-Control Meta Tags to Head to prevent browser/CDN stale caching
...
if 'http-equiv="Cache-Control"' not in content:
    if '<head>' in content:
        ...
        print('Injected Cache-Control meta tags')

# 2. Add Skull 💀 Favicon with cache-buster
SKULL_FAVICON_TAGS = """<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>💀</text></svg>" />
<link rel="shortcut icon" href="/favicon.ico?v=999" />
<link rel="apple-touch-icon" href="/favicon.svg?v=999" />"""

# Clean old favicon links
...
if '<head>' in content:
    content = content.replace('<head>', '<head>\n' + SKULL_FAVICON_TAGS, 1)
    print('Injected cache-busting Skull 💀 favicon links')

# 3. Clean any old injection script
...
# 4. Bulletproof Fixed Top-Right Positioning (NO OVERLAP WITH LEFT LOGO!)
DEEP_FIX_SCRIPT = r"""<script>
(function () {
  /* PARAMVEER CREDIT POPUP - Top-Right Fixed Pill & Slide Panel (NO OVERLAP) */

  function injectTopRightPill() {
    ...
    /* FIXED TOP-RIGHT POSITIONING - Guaranteed to NEVER touch or overlap the left logo! */
    ...
    pill.onmouseenter = function() {
      this.style.background = 'rgba(40, 40, 48, 0.95)';
      this.style.borderColor = 'rgba(255, 255, 255, 0.5)';
    };
    pill.onmouseleave = function() {
      this.style.background = 'rgba(20, 20, 24, 0.85)';
  ...
  function injectSlidePanel() {
      ...
      '<div style="position:relative;max-width:820px;margin:0 auto;padding:36px 40px 32px;box-sizing:border-box;">' +
        ...
        '<div style="display:flex;align-items:center;gap:18px;margin-bottom:18px;">' +
          ...
          '<div>' +
            ...
            '<p style="margin:0;font-size:13px;color:#a1a1aa;">Full-Stack Developer &bull; Creator of OmniRip &bull; zalazalazalaparamveer113@gmail.com</p>' +
          '</div>' +
        '</div>' +
        ...
        '<div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;">' +
          ...
          '</a>' +
          '<a id="pz-btn-email" href="mailto:zalazalazalaparamveer113@gmail.com" style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.08);color:#d4d4d8;padding:12px 22px;border-radius:9999px;font-size:14px;font-weight:500;text-decoration:none;border:1px solid rgba(255,255,255,0.14);transition:all 0.2s;" onmouseover="this.style.background=\'rgba(255,255,255,0.14)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.08)\'">' +
            '&#9993; Email' +
          '</a>' +
        '</div>' +
      '</div>';
    ...
    if (document.body) {
      ...
      document.getElementById('pz-btn-email').addEventListener('click', function(e) {
        e.stopPropagation();
        window.location.href = 'mailto:zalazalazalaparamveer113@gmail.com';
      });
    }
  }
  ...
  /* Handle Pill Clicks via Global Capture Delegation */
  document.addEventListener('click', function (e) {
    var el = e.target;

    // Ignore clicks originating inside the slide panel
    if (el && el.closest('#pz-slide-panel')) {
      return; 
    }
...
if '</body>' in content:
    content = content.replace('</body>', DEEP_FIX_SCRIPT + '\n</body>', 1)
    print('Injected fixed top-right pill & slide panel script before </body>')
